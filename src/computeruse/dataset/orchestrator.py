...
from .registry import AppConfig, AppState, DriveStep, load_registry
...
def run(
    ...
    for app_config in apps:
        pending = [s for s in app_config.states if not ledger.is_done(app_config.name, s.name)]
        if not pending:
            logger.info(
        ...
        for state in pending:
            ...
            try:
                count = _run_state(app_config, state, controller, dataset_root, session_id, seq)
                ledger.mark_done(app_config.name, state.name)
                logger.info(
                ...
                logger.error("app=%s state=%s failed, skipping state: %s", app_config.name, state.name, exc)
