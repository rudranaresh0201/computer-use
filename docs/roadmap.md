...
- [ ] **Phase 3 (weeks 4–9) — GUI Grounding Model: the research core** (ADR-0003)
  ...
  - Dataset collection: **14/14 registry apps collected, frozen as v2 on 2026-07-16** after v1 (8/14, 2026-07-13) and a full re-collection that fixed four real pipeline bugs (chrome-vs-content sampling bias, a too-shallow UIA depth cutoff, a packaged-app rect race, and two PII leaks) — dataset card at `data/gui_grounding/README.md` (570 examples, 32 screenshots, 0 integrity issues). Held-out pool now spans all 4 held-out apps — H3 finally has real cross-app diversity, though the evaluation itself (line below) still isn't built.
